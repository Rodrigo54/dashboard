...
import { mergeClasses, transform } from '@/shared/ui/zard/utils/merge-classes';
...
export class ZardSelectComponent
  implements FormValueControl<string | string[]>, ControlValueAccessor, OnDestroy
{
  ...
  readonly value = model<string | string[]>(this.zMultiple() ? [] : '');
  ...
  afterRenderEffect = afterRenderEffect(() => {
    ...
    for (const item of this.selectItems()) {
      item.setSelectHost({
        selectedValue: () =>
          this.zMultiple() ? (this.value() as string[]) : [this.value() as string],
  ...
  selectItem(value: string, label: string) {
    ...
    this.onChange(value);
    this.zSelectionChange.emit(this.value());
